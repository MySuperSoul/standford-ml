...
for i = 1:m
    ...
    z3 = a2 * Theta2';
    a3 = sigmoid(z3);
    first_item = -vec_y .* (log(a3))';

    second_item = (1 - vec_y) .* log(1 - a3)';
    item = first_item - second_item;
    J = J + sum(item);

    % compute gradient for every theta
    a3 = a3';
    a2 = a2';
    exp3 = a3 - vec_y;
    exp2 = Theta2' * exp3 .* a2 .* (1 - a2);

    exp2 = exp2(2:length(exp2));

    Theta2_grad = Theta2_grad + exp3 * a2';
    Theta1_grad = Theta1_grad + exp2 * ith_x;
end

J = 1.0 * J / m;
J = J + lambda / (2.0 * m) * (theta1_sum + theta2_sum);
